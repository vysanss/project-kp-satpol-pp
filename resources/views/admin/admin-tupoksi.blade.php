...
<head>
    ...
    <title>Kelola Tupoksi - Admin</title>
...
<body class="bg-gray-50 pt-20 min-h-screen">
            ...
            <div>
                ...
                <div class="mt-4 flex items-center space-x-4">
                    ...
                    <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                        <span class="text-sm font-medium">Tugas: {{ $tupoksi->where('kategori', 'Tugas')->count() }}</span>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                        <span class="text-sm font-medium">Fungsi: {{ $tupoksi->where('kategori', 'Fungsi')->count() }}</span>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                        <span class="text-sm font-medium">Bidang Kerja: {{ $tupoksi->where('kategori', 'Bidang Kerja')->count() }}</span>
                    </div>
                </div>
            </div>
    ...
    @php
        $kategoriList = ['Tugas', 'Fungsi', 'Bidang Kerja'];
    @endphp
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="bg-white rounded-lg shadow p-6 flex-1 flex flex-col">
                ...
                <div class="overflow-x-auto">
                    <!-- Tambahkan max-h dan overflow-y-auto pada wrapper tabel -->
                    <div class="max-h-64 overflow-y-auto">
                        <table class="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <tbody>
                                @php $no = 1; @endphp
                                @foreach($tupoksi->where('kategori', $kategori) as $row)
                                <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }} hover:bg-blue-50 transition">
                                    ...
                                    <td class="px-4 py-3 border-b text-center">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <!-- Hapus -->
                                            <form action="{{ route('admin.tupoksi.destroy', $row->id) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">
                                                @csrf
                                                @method('DELETE')
                ...
                @if($tupoksi->where('kategori', $kategori)->count() == 0)
    ...
    <div id="tambahModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
            ...
            <form id="tambahForm" action="{{ route('admin.tupoksi.store') }}" method="POST" class="flex flex-col gap-3">
                @csrf
    ...
    function openEditModal(id, poin, isi, kategori) {
        ...
        form.action = "{{ url('admin/tupoksi') }}/" + id;
    }
